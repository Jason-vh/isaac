...
export function computeCriticalPath(
  ...
  if (totalMs <= 0) return { criticalJobs: new Set() };

  // Step 1: build name→job map, only non-retried jobs with timestamps
  const jobMap = new Map<string, JobInput>();
  for (const j of jobs) {
  ...
  // Use queue-adjusted start so runner queue time doesn't appear as slack.
  // Also use the earliest start across ALL attempts (including retries).
